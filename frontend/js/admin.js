...
async function loadBookings() {
    try {
        ...
        bookings.forEach(b => {
            ...
            tbody.innerHTML += `<tr>
                <td><strong>${b.user.full_name}</strong></td>
                <td><code>${b.user.identity_number}</code></td>
                <td>${b.room.room_number} <span class="text-muted">(${b.room.type})</span></td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('addRoomForm').addEventListener('submit', async(e)=>{
        ...
        try{
            ...
            if(file){const fd=new FormData();fd.append('image',file);const ur=await fetch('/api/rooms/upload',{method:'POST',headers:{'Authorization':`Bearer ${token}`},body:fd});if(!ur.ok)throw new Error('Upload failed');const ud=await ur.json();image_url=ud.image_url;}
